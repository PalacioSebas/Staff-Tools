...
class MinecraftStaffToolsApp(QMainWindow):
    def __init__(self):
        ...
        self.clics_proximamente = 0           #Contador Easter Egg
    ...
    def no_disponible(self):
        """Mensaje para herramientas no disponibles (y trigger del Easter Egg)."""
        
        # Si el easter egg ya está activo, cualquier clic lo desactiva
        if theme_manager.easter_egg_active:
            theme_manager.easter_egg_active = False
            self.clics_proximamente = 0
            central = self.centralWidget()
            if central:
                central.setStyleSheet(theme_manager.get_background_style())
            theme_manager.notify_all()
            return

        self.clics_proximamente += 1

        if self.clics_proximamente == 3:
            ruta_actual = os.path.dirname(os.path.abspath(__file__))
            ruta_foto = os.path.join(ruta_actual, "secreto.jpg")
            
            if theme_manager.activar_easter_egg(ruta_foto):
                central = self.centralWidget()
                if central:
                    central.setStyleSheet(theme_manager.get_background_style())
                theme_manager.notify_all()
            else:
                print("Falta el archivo secreto.jpg en la carpeta.")
                self.clics_proximamente = 0
            return
        QMessageBox.warning(
            self,
