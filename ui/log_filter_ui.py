from PySide6.QtWidgets import (QDialog, QVBoxLayout, QLabel, QPushButton, 
                               QLineEdit, QFileDialog, QMessageBox, QCheckBox, QWidget)
...
class LogFilterWindow(QDialog):
    ...
    def crear_ui(self):
        ...
        container.setStyleSheet(theme_manager.get_background_style())       
        ...
        btn_archivo.setMinimumHeight(45) # Aumentamos de 35 a 45
        btn_archivo.setCursor(Qt.PointingHandCursor)
        estilo_boton = theme_manager.get_button_style()
        btn_archivo.setStyleSheet(estilo_boton + """
            QPushButton {
                padding-left: 15px;
                padding-right: 15px;
                font-size: 11pt; /* Ajustamos el tamaño si es necesario */
            }
        """)
        ...
        self.lbl_archivo.setMinimumHeight(30)
        self.lbl_archivo.setWordWrap(True)        
        self.lbl_archivo.setStyleSheet(f"color: {theme_manager.get_text_alpha(0.8)}; background: transparent; padding: 5px;")
        ...
        self.chk_tpakill = QCheckBox("Modo Investigación TPAKILL")
        self.chk_tpakill.setToolTip("Incluye todos los mensajes de TPA y muertes, aunque no nombren al jugador.")
        self.chk_tpakill.setStyleSheet(f"""
            QCheckBox {{
                color: {theme_manager.get_text_color()};
                font-weight: bold;
            ...
            QCheckBox::indicator {{
                width: 18px;
                height: 18px;
                border: 2px solid {theme_manager.get_text_color()};
                border-radius: 3px;
            }}
            QCheckBox::indicator:checked {{
                background-color: {theme_manager.get_text_color()};
            }}
        """)
        layout.addWidget(self.chk_tpakill)
    ...
    def ejecutar_filtro(self):
        ...
        activar_tpakill = self.chk_tpakill.isChecked() 
        
        filtro = FiltroLogs(self.archivo_log)
        

        filtro.filtrar_por_jugadores(jugadores, modo_tpakill=activar_tpakill)
    ...
    def aplicar_tema(self):
        """Aplica el tema actual al fondo de esta ventana."""
        self.setStyleSheet(theme_manager.get_background_style())
    
    def closeEvent(self, event):
        """Al cerrar, desregistrarse del theme_manager."""
